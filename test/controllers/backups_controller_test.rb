...
class BackupsControllerTest < ActionController::TestCase
  ...
  test "should create backup" do
    assert_difference('Backup.count') do
      post :create, backup: { sourcehost: @backup.sourcehost, sourcepath: @backup.sourcepath, status: @backup.status, targethost: @backup.targethost, targetpath: @backup.targetpath }
    end
  ...
  test "should update backup" do
    patch :update, id: @backup, backup: { sourcehost: @backup.sourcehost, sourcepath: @backup.sourcepath, status: @backup.status, targethost: @backup.targethost, targetpath: @backup.targetpath }
    assert_redirected_to backup_path(assigns(:backup))
  end
